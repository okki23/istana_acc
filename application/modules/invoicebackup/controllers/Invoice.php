...
class invoice extends Parent_Controller {
  
  var $nama_tabel = 't_invoice';
  ...
  var $primary_key = 'id';
 	...
 	public function __construct(){
	   ...
	   public function index(){
		  ...
		  $data['konten'] = 'invoice/invoice_view';
  	...
  	public function fetch_invoice_list(){  
       $getdata = $this->m_invoice->fetch_invoice_list();
       echo json_encode($getdata);   
    }
    
    public function setup_resi(){
      $no_resi = $this->input->post('no_resi');
      $no_invoice = $this->input->post('no_invoiceresi');
      $this->db->query("update t_invoice set no_resi = '".$no_resi."' where no_invoice = '".$no_invoice."' ");
	  $sqlcust = $this->db->query("select a.*,b.full_name,b.email,b.alamat,b.telp from t_invoice a
left join m_customer b on b.id = a.id_customer where a.no_invoice = '".$no_invoice."' ")->row();
	  $no_resi = $this->input->post('no_resi');
		
		$isiEmail = '<h1 align="center"> Invoice : '.$no_invoice.'</h1> <br>
		<p> Diberitahukan kepada pemilik Invoice ini yang bernama : <br>
		
		<b>
		Nama : '.$sqlcust->full_name.'
		<br>
		Alamat : '.$sqlcust->alamat.'
		<br>
		Telp : '.$sqlcust->telp.'
		<br>
		</b>
		
		Dengan ini kami sampaikan barang anda sedang dalam pengiriman ke alamat anda, anda dapat melacak
		pesanan anda dengan nomor resi : '.$no_resi.' <br>
        ...
        $mail->SMTPAuth   = true;  
        $mail->SMTPSecure = "ssl";  // 
        $mail->Host       = "mail.mingguskripsi.com";   
       
        $mail->Port       = 465;  
      
        $mail->Username   = "istanacc@mingguskripsi.com"; 
        $mail->Password   = "Wid_2019OKE!";     
        $mail->SetFrom('istanacc@mingguskripsi.com', 'noreply');  
        $mail->Subject    = "Informasi Nomor Resi untuk Invoice : ".$no_invoice;
        $mail->Body       = $isiEmail;
        $toEmail = $sqlcust->email;
    ...
    public function listingdetail(){  
      ...
      $sql = "select a.*,b.nama_barang,c.nama_kategori from t_invoice_detail a
    ...
    public function calc_weight(){
       ...
       $res = 0;
       $sql = $this->db->query(" 
                  SELECT a.*,b.harga,c.nama_produk,d.berat_bahan FROM t_invoice_detail a
    ...
    public function list_order_customer(){
       $getdata = $this->m_invoice->list_order_customer();
       echo json_encode($getdata);   
    }

    public function list_invoicedetail_bycustomer(){  
    ...
    public function hapus_no_order(){
      $no_order = $this->input->post('no_order');
      echo $no_order;
      $this->db->query("delete from t_invoice where no_order = '".$no_order."' ");

    }
 
     
	  public function transaksi_id($param = '') {
        $data = $this->m_invoice->get_no();
        $lastid = $data->row();
        $idnya = $lastid->id;
    ...
    public function invoice_id($param = '') {
        $data = $this->m_invoice->get_invoice_no();
        $lastid = $data->row();
        $idnya = $lastid->id;
  ...
  public function hapus_data(){
    ...
    $delete_invoice = $this->db->where('no_invoice',$no_invoice)->delete('t_invoice');
    $delete_invoice_detail = $this->db->where('no_invoice',$no_invoice)->delete('t_invoice_detail');
     
    if($delete_invoice && $delete_invoice_detail){
      $result = array("response"=>array('message'=>'success')); 
    }else{
      $result = array("response"=>array('message'=>'failed'));
    }
  ...
  echo json_encode($result,TRUE);
   ...
   public function simpan_data_bb(){
    ...
    $data_form = $this->m_invoice->array_from_post($this->daftar_field);
    $bukti_bayar_upload = $this->bukti_bayar_upload();
    $sql = $this->db->query("update t_invoice set bukti_bayar_upload = '".$bukti_bayar_upload_file."', bukti_bayar_upload_date = '".date('Y-m-d H:i:s')."',  status ='2' where no_order = '".$no_ordery."' ");
    ...
    if($id == '' || $id == NULL){
    	//query insert
    	$sql = $this->db->query("insert into t_invoice (no_order,id_pelanggan,date_create,berkas_file,status) values ('".$data_form['no_order']."','".$data_form['id_pelanggan']."','".$data_form['date_create']."','".$data_form['berkas_file']."','1')");
    }else{
    	$sql = $this->db->query("update t_invoice set no_order = '".$data_form['no_order']."', id_pelanggan = '".$data_form['id_pelanggan']."', berkas_file = '".$data_form['berkas_file']."' where id = '".$id."' ");
    }

    //$simpan_data = $this->m_invoice->simpan_data($data_form,$this->nama_tabel,$this->primary_key,$id);
  ...
  public function fetch_invoicelist_table(){
    $getdata = $this->m_invoice->fetch_invoicelist_table();
    echo json_encode($getdata);
  }
  ...
  public function simpan_data_detail(){
    ...
    $data_form = $this->m_invoice->array_from_post($field); 
